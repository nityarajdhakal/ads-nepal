...
const sendContactNotification = async ({ firstName, lastName, email, phone, service, message }) => {
  const notifyEmail = process.env.NOTIFY_EMAIL || process.env.EMAIL_USER;
  if (!notifyEmail || !process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
    throw new Error('Email configuration is missing. Set EMAIL_USER, EMAIL_PASS, and NOTIFY_EMAIL.');
  }
...
const submitContact = async (req, res) => {
  try {
    ...
    // Send notification in background so submit response stays fast.
    sendContactNotification({ firstName, lastName, email, phone, service, message })
      .catch((emailError) => {
